...
<script>
   $(function() {
      ...
      $('#btMultiDel').click(function() {
         ...
         $('form[name=frmList]').prop("action","<c:url value='/scholarship/scholarshipDel'/>");
         $('form[name=frmList]').submit();
...
<div class="container">
		<div id="adminMngMem">
			<div class="divTop">
				...
				<!-- 페이징 처리를 위한 form 시작-->
				<form name="frmPage" method="post" action="<c:url value='/scholarship/scholarshipList'/>">
			...
			<form name="frmList" method="post" action="<c:url value='/scholarship/scholarshipWrite'/>">
				<div class="divRight">
					<div class="divTop">
						<div class="stud">
								...
								<c:if test="${!empty list }">
									...
									<c:forEach var="map" items="${list }">
										<tr>
											...
											<td><a
												href="<c:url value='/scholarship/scholarshipEdit?officialNo=${map["STU_NO"] }'/>">수정</a></td>
											<td><a id="deleteBt"
												href="<c:url value='/scholarship/scholarshipDeleteStudent?stuNo=${map["STU_NO"] }'/>">삭제</a></td>
										</tr>
					...
					<div class="btdiv">
						<input type="button"
							...
							onclick="location.href='<c:url value="/scholarship/scholarshipWrite?sort=3"/>'"
							id="btInsert" value="장학금 추가"><br>
					</div>
					<div class="btdiv">
						<input type="button" class="btCustom btn btn-primary btn-lg login-button" id="btMultiDel" value="선택한 학생 삭제"><br>
					</div>
				</div>
			</form>
		</div>
